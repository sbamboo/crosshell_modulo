# [EnsautopipImporturePIPPack]
import os
from cslib import fromPathAA,autopipImport
...
from cslib.externalLibs.tabledraw import drawTable
...
def downloadRepositoryVerfFile(verfFile=str(), verfURL=str()):
    try:
        ...
        downloadFile_HandleGdriveVirWarn(
            ...
            filepath=verfFile,
            loadingBar=True,
            title="Downloding repo verification...",
            onFileExiError="remove"
        )
    except:
...
def updateRepositoryFile(repoFile=str(),versionCheck=bool(),skipOnEmptyURL=False,ignoreFormat=False,verfFile=None,repoURL=None,verfURL=None,localFormatVersion=None):
        ...
        if repoURL != None:
            ...
            if versionCheck == True:
                if verfFile != None and verfURL != None:
                    #if os.path.exists(verfFile): os.remove(verfFile)
                    #simpleDownload(verfURL,verfFile)
                    downloadRepositoryVerfFile(verfFile,verfURL)
        else:
    ...
    # Should be updated
        ...
        if versionCheck == True:
            ...
            ph_onlineVerf_raw = getUrlContent_HandleGdriveVirWarn(
                url=verfURL,
                loadingBar=True,
                title="Downloding repo file..."
            ).decode()
